...
<div id = 'bg_middle2'>
	<div class = 'apparel_top_title'>
    	<h3><a href='../../index.php'>Home</a> > <a href='#'>Apparel</a> > <a href='#'>Women</a></h3> 
	</div>
...
<div id="apparel_content_page_wrapper">
    ...
    <!---------------------------------------------------------->
		<div id='left_menu'><br/>
    	<ul>
        	<li id='lin'>WOMEN CLOTHES</li>
        <br/>
        </ul>
        <ul>
        	<li id='lin'>Categories</li>
            <a href='?wlink=Women' class='active'><li>Appends</li></a>
            <a href='?wlink=Women_Dresses'><li>Dresses</li></a>
            <a href='?wlink=Women_Tops'><li>Tops</li></a>
    		<a href='?wlink=Women_T-Shirts'><li>T-Shirts</li></a>
            <a href='?wlink=Women_Pants'><li>Pants</li></a>
            <a href='?wlink=Women_Skirts'><li>Skirts</li></a>
            <a href='?wlink=Women_Active-wear'><li id='lil'>Active Wear</li></a>
   		</ul>
        <ul>
  ...
  <!------------------------>
    ...
    <!------------------------>
<?php 
 if(isset($_GET['wlink'])){$sub_category = $_GET['wlink'];}else{$sub_category = 'Women';} //for form
    ...
    <div id = 'index_content_page_wrapper2'> ";
        ...
        <form action='women.php' method='GET'>
        	<label>Sort By:</label> 
		...
		<table>
        	<tr>";
				/*** SET UP pagination  ***/ 
				$item_per_page = 9; 
				if(isset($_GET['wlink'])){$sub_category= $_GET['wlink'];}else{$sub_category='Women';}
				...
				if((isset($_GET['wlink'])) || (isset($_GET['order_by'])))//if url has a variable
				{
					...
					if($sub_category == 'new') {$sub_category = 'Women'; $order = 'item_id DESC'; }
					if($sub_category == 'like'){$sub_category = 'Women'; $order = 'thumb_up DESC'; }
					...
					$women_query = mysql_query("SELECT * FROM item WHERE sub_category LIKE '%$sub_category%' ORDER BY $order 
													LIMIT $start, $item_per_page"); 
				}
				else//no url variable and no sumbit button
				{
					$order_by_p = 'heigh_low';
					$women_query = mysql_query("SELECT * FROM item WHERE sub_category LIKE '%$sub_category%' ORDER BY price DESC LIMIT $start, $item_per_page");  
				}
